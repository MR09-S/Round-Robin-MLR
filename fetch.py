...
def get_enhanced_metrics(sample_duration=1):
    ...
    time.sleep(0.1)  # Short window for I/O measurement
    io_after = psutil.disk_io_counters()
    metrics["io_bytes_sec"] = (io_after.read_bytes + io_after.write_bytes - io_before.read_bytes - io_before.write_bytes) * 10  # Scale to 1s
    ...
    try:
        ...
        metrics["context_switches_sec"] = (ctx_after - ctx_before) * 10
    except AttributeError:
        metrics["context_switches_sec"] = 0  # Unsupported on Windows
...
if __name__ == "__main__":
    log_enhanced_metrics(sample_interval=1)